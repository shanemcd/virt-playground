...
The SSH tunnel is worth understanding: `virtctl ssh` makes an API request to the Kubernetes API server, which proxies the connection through to virt-handler on the node, which connects to the virt-launcher pod's SSH port. The SSH traffic tunnels through the API server rather than requiring direct network access to the pod IP.
